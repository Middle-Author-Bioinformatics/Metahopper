...
Main controls:
  ...
  --rounds N                  default 1: one guarded frontier-extension round
...
def write_run_manifest(outdir: Path, args) -> None:
    ...
    payload = {
        ...
        "workflow": {key: getattr(args, key) for key in (
            ...
            "seed_mapper", "seed_tie_policy", "bwa_seed_length", "bwa_mismatch_penalty",
            "bwa_gap_open_penalty", "bwa_min_score")},
...
def run_recruited_qc(r1: Path, r2: Path, input_single: Path, outdir: Path, threads: int,
                      trimmomatic_folder: Path, qc_quality: int, qc_minlen: int,
                      flash_max_overlap: int) -> tuple:
    """QC a recruited read pool without discarding singletons extracted with the pairs.

    ``run_qc`` historically accepted only R1/R2.  Recruitment, however, deliberately
    extracts an additional singleton FASTQ.  Earlier MetaHopper versions deleted that
    file after paired-read QC, so those recruited reads never reached SPAdes/Unicycler.
    This wrapper preserves them, adapter/quality trims them with Trimmomatic SE, and
    appends them to the merged/orphan singleton stream produced by ``run_qc``.
    """
    paired_r1, paired_r2, generated_single = run_qc(
        r1, r2, outdir, threads,
    ...
    if not input_single or not Path(input_single).is_file() or count_fastq_reads(input_single) == 0:
        return paired_r1, paired_r2, generated_single

    adapters = Path(trimmomatic_folder) / "adapters" / "TruSeq3-PE-2.fa"
    cleaned_single = outdir / "recruited_input_single.cleaned.fastq.gz"
    run_cmd([
        "trimmomatic", "SE", "-threads", str(threads), str(input_single), str(cleaned_single),
        f"ILLUMINACLIP:{adapters}:2:30:10", f"SLIDINGWINDOW:4:{qc_quality}",
        f"MINLEN:{qc_minlen}",
    ], log_file=outdir / "recruited_single_qc.log")
    combined_single = outdir / "unpaired.with_recruited_single.fq.gz"
    combine_fastqs(combined_single, [generated_single, cleaned_single])
    cleaned_single.unlink(missing_ok=True)
    return paired_r1, paired_r2, combined_single
...
# pool is reassembled alone. MetaHopper 7 defaults to an auto strategy that evaluates
# direct SPAdes first and, when needed, compares an Unicycler candidate. A subset-only
# assembly graph is far simpler than the whole-community graph, so it can often resolve
# tangles (shared genes, similar-coverage strains) that fragmented the original bin. Frontier extension is
...
def build_bwa_index(fasta: Path, log_file: Path) -> None:
    """Build a BWA index beside ``fasta``."""
    run_cmd(["bwa", "index", str(fasta)], log_file=log_file)


def map_reads_with_bwa(r1: Path, r2: Path, reference: Path, out_bam: Path, threads: int,
                       seed_length: int, mismatch_penalty: int, gap_open_penalty: int,
                       min_score: int, log_file: Path) -> None:
    """BWA-MEM competitive seed mapping.

    ``-a`` is important here: MetaHopper needs secondary alignments so a read that fits
    multiple taxonomic bins can be treated as ambiguous instead of being assigned from
    only BWA's primary placement.  The defaults reproduce the stringent Verdanus
    reassembly settings (``-k 50 -B 10 -O 10 -T 90``), but are exposed as CLI options.
    Unmapped records are retained so later frontier recruitment can still extract their
    mates by template name.
    """
    bwa = [
        "bwa", "mem", "-a", "-t", str(threads),
        "-k", str(seed_length), "-B", str(mismatch_penalty),
        "-O", str(gap_open_penalty), "-T", str(min_score),
        str(reference), str(r1), str(r2),
    ]
    sam = ["samtools", "view", "-@", str(threads), "-b", "-o", str(out_bam), "-"]
    run_pipeline([bwa, sam], log_file=log_file)
...
def competitively_assign_templates(bam: Path, ref_to_bin: dict, min_aligned_fraction: float,
                                   min_identity: float, threads: int,
                                   tie_policy: str = "exclude") -> tuple:
        ...
        if len(winners) != 1:
            ...
            if tie_policy == "include":
                for winner in winners:
                    assignments[winner].add(template)
            continue
...
def assemble_recruited_pool(r1: Path, r2: Path, single: Path, workdir: Path, threads: int,
                             ...
                             bin_label: str, unicycler_argv=None,
                             original_fasta: Path = None,
                             min_recovered_fraction: float = 0.95) -> tuple:
    """Assemble one recruited pool and return the best candidate.

    ``auto`` is the MetaHopper 7 default.  It tries direct SPAdes first because the
    Verdanus reassembly workflow showed that a focused read pool can assemble well with
    SPAdes alone.  If that candidate does not already pass the sequence-retention and
    contiguity gates, Unicycler is also tried and the two candidates are compared against
    the original bin.  This prevents a merely *successful* but highly incomplete
    Unicycler assembly from suppressing a better direct-SPAdes reconstruction.
    """
    workdir.mkdir(parents=True, exist_ok=True)

    def evaluate(candidate, source, circular_count=0):
        if candidate is None or not Path(candidate).is_file() or Path(candidate).stat().st_size == 0:
            return None
        stats = basic_assembly_stats(candidate)
        retention = None
        ratio = None
        sequence_gate = False
        contiguity_gain = False
        if original_fasta is not None:
            before = basic_assembly_stats(original_fasta)
            evdir = workdir / f"candidate_eval_{sanitize(source)}"
            evdir.mkdir(parents=True, exist_ok=True)
            retention = original_retention(original_fasta, candidate, evdir, threads)
            ratio = stats["total_length_bp"] / max(1, before["total_length_bp"])
            sequence_gate = (retention >= min_recovered_fraction and
                             ratio >= min_recovered_fraction and ratio <= 1.25)
            contiguity_gain = (stats["num_contigs"] <= before["num_contigs"] * 0.9 or
                               stats["N50"] >= max(1, before["N50"]) * 1.2)
        # Sequence preservation dominates the ranking; contiguity matters only after that.
        score = (int(sequence_gate), int(contiguity_gain), retention or 0.0,
                 stats["N50"], -stats["num_contigs"], stats["total_length_bp"])
        return dict(path=Path(candidate), source=source, circular=circular_count,
                    stats=stats, retention=retention, ratio=ratio,
                    sequence_gate=sequence_gate, contiguity_gain=contiguity_gain,
                    score=score)

    candidates = []

    def run_spades(label="spades"):
        try:
            path = run_spades_targeted(
                r1, r2, single, workdir / label, threads, spades_memory_gb,
                spades_mode, kmers, workdir / f"{label}.log",
            )
            return evaluate(path, label, 0)
        except RuntimeError as exc:
            log.warning("Bin '%s': direct SPAdes assembly failed (%s).", bin_label, exc)
            return None

    def run_unicycler(label="unicycler"):
        if unicycler_argv is None:
            return None
        path, circular = run_unicycler_assembly(
            r1, r2, single, workdir / label, threads, unicycler_mode, kmers=kmers,
            extra_args=unicycler_extra, unicycler_argv=unicycler_argv,
        )
        return evaluate(path, label, circular)

    if assembler == "spades":
        candidate = run_spades("spades")
        if candidate:
            candidates.append(candidate)
    elif assembler == "unicycler":
        candidate = run_unicycler("unicycler_run")
        if candidate:
            candidates.append(candidate)
        elif spades_fallback:
            log.warning("Bin '%s': Unicycler produced no assembly; trying direct SPAdes.", bin_label)
            candidate = run_spades("spades_fallback")
            if candidate:
                candidates.append(candidate)
    else:  # auto
        spades_candidate = run_spades("spades_auto")
        if spades_candidate:
            candidates.append(spades_candidate)
        # If SPAdes already preserves the source sequence and improves contiguity, avoid
        # paying for a second assembler.  Otherwise give Unicycler a chance and compare.
        need_unicycler = not (spades_candidate and spades_candidate["sequence_gate"] and
                              spades_candidate["contiguity_gain"])
        if need_unicycler:
            unicycler_candidate = run_unicycler("unicycler_auto")
            if unicycler_candidate:
                candidates.append(unicycler_candidate)

    if not candidates:
        return None, None, 0

    chosen = max(candidates, key=lambda x: x["score"])
    table = workdir / "assembly_candidates.tsv"
    with open(table, "w", newline="") as fh:
        w = csv.writer(fh, delimiter="\t")
        w.writerow(["assembler", "selected", "sequence_gate", "contiguity_gain",
                    "original_retention", "length_ratio", "contigs", "total_length_bp", "N50"])
        for row in candidates:
            w.writerow([
                row["source"], int(row is chosen), int(row["sequence_gate"]),
                int(row["contiguity_gain"]),
                "NA" if row["retention"] is None else f"{row['retention']:.6f}",
                "NA" if row["ratio"] is None else f"{row['ratio']:.6f}",
                row["stats"]["num_contigs"], row["stats"]["total_length_bp"],
                row["stats"]["N50"],
            ])
    log.info(
        "Bin '%s': selected %s candidate (retention=%s, length_ratio=%s, N50=%s, contigs=%s).",
        bin_label, chosen["source"],
        "NA" if chosen["retention"] is None else f"{chosen['retention']:.3f}",
        "NA" if chosen["ratio"] is None else f"{chosen['ratio']:.3f}",
        chosen["stats"]["N50"], chosen["stats"]["num_contigs"],
    )

    assembly = chosen["path"]
    source = chosen["source"]
    circular_count = chosen["circular"]
    if polish:
        polished = run_pilon_polish(assembly, r1, r2, single, workdir, threads)
        if polished is not None:
            assembly = polished
            source = f"{source}+pilon"
...
def reassemble_one_bin(bin_fasta: Path, bin_name: str, rank: str, r1_raw: Path, r2_raw: Path,
    ...
    seed_r1, seed_r2, seed_single = run_recruited_qc(
        seed_raw_r1, seed_raw_r2, seed_raw_single, workdir / "qc" / "seed", threads,
        trimmomatic_folder, qc_quality, qc_minlen, flash_max_overlap,
    )
    ...
    if max_rounds > 0:
        ...
        for round_n in range(1, max_rounds + 1):
            ...
            clean_r1, clean_r2, clean_single = run_recruited_qc(
                raw_r1, raw_r2, raw_single, round_dir / "qc", threads,
                trimmomatic_folder, qc_quality, qc_minlen, flash_max_overlap,
            )
    ...
    chosen_contigs, chosen_stage, circular_count = assemble_recruited_pool(
        ...
        unicycler_argv=unicycler_argv, original_fasta=bin_fasta,
...
def run_bin_reassembly(outdir: Path, ranks, r1_raw: Path, r2_raw: Path, threads: int,
                        ...
                        checkm_options=None, preliminary_classifications=None, bin_minutes=0,
                        seed_mapper: str = "bowtie2", seed_tie_policy: str = "exclude",
                        bwa_seed_length: int = 50, bwa_mismatch_penalty: int = 10,
                        bwa_gap_open_penalty: int = 10, bwa_min_score: int = 90) -> dict:
    global BIN_DEADLINE
    ...
    for rank in ranks:
        ...
        if eligible:
            ...
            fingerprint = dict(
                seed_sha256=sequence_digest(seed), r1=file_signature(r1_raw),
                r2=file_signature(r2_raw), mapper=seed_mapper, score=seed_score_min,
                insert=max_insert, bwa_seed_length=bwa_seed_length,
                bwa_mismatch_penalty=bwa_mismatch_penalty,
                bwa_gap_open_penalty=bwa_gap_open_penalty, bwa_min_score=bwa_min_score,
                tie_policy=seed_tie_policy, mapping_version=4,
            )
            ...
            if not reusable:
                ...
                if seed_mapper == "bwa":
                    build_bwa_index(seed, cd / "bwa-index.log")
                    map_reads_with_bwa(
                        r1_raw, r2_raw, seed, bam, threads,
                        bwa_seed_length, bwa_mismatch_penalty, bwa_gap_open_penalty,
                        bwa_min_score, cd / "bwa.log",
                    )
                else:
            ...
            with open(cd / "competitive_mapping.tsv", "w") as fh:
                ...
                fh.write(f"{rank}\t__AMBIGUOUS_TIES__\t{ambiguous}\n")
                fh.write(f"{rank}\t__TIE_POLICY_{seed_tie_policy.upper()}__\t0\n")
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="MetaHopper 7: classify and bin contigs; with reads, run guarded read-recruitment and reassembly. "
    ...
    rb.add_argument("--seed-mapper", choices=["bowtie2", "bwa"], default="bowtie2",
                     help="Mapper for competitive seed recruitment (default bowtie2). 'bwa' uses "
                          "BWA-MEM and exposes the stringent Verdanus-style parameters below.")
    rb.add_argument("--seed-tie-policy", choices=["exclude", "include"], default="exclude",
                     help="What to do with templates tied for best score across bins (default "
                          "exclude). 'include' gives tied templates to every tied best bin; this "
                          "is more sensitive but less conservative.")
    rb.add_argument("--reassemble-seed-score-min", default="G,20,8",
                     help="Bowtie2 --score-min for seed recruitment (default G,20,8; ignored with BWA).")
    rb.add_argument("--bwa-seed-length", type=int, default=50,
                     help="BWA-MEM -k minimum seed length (default 50; Verdanus setting).")
    rb.add_argument("--bwa-mismatch-penalty", type=int, default=10,
                     help="BWA-MEM -B mismatch penalty (default 10; Verdanus setting).")
    rb.add_argument("--bwa-gap-open-penalty", type=int, default=10,
                     help="BWA-MEM -O gap-open penalty (default 10; Verdanus setting).")
    rb.add_argument("--bwa-min-score", type=int, default=90,
                     help="BWA-MEM -T minimum output score (default 90; Verdanus setting).")
    ...
    rb.add_argument("--reassemble-max-rounds", type=int, default=1,
                     help="Exact-kmer frontier-extension rounds after competitive seed "
                          "recruitment (default 1; pass 0 for seed-only reassembly).")
    ...
    rb.add_argument("--reassemble-bait-min-entropy", type=float, default=0.45,
                     help="Excludes low-complexity bait sequence from frontier extension "
                          "(default 0.45).")
    rb.add_argument("--reassemble-max-round-growth", type=float, default=1.0,
                     help="Reject an extension round if new/accepted exceeds this fraction. "
                          "MetaHopper 7 allows the first guarded round to at most double the pool "
                          "(default 1.0); lower this for very complex communities.")
    rb.add_argument("--reassemble-max-accepted-fraction", type=float, default=0.20,
                     help="Reject an extension round if accepted/total-raw-reads would exceed "
                          "this fraction (default 0.20; accommodates abundant insect symbionts "
                          "while retaining a hard anti-snowballing cap).")
    ...
    rb.add_argument("--assembler", choices=["auto", "unicycler", "spades"], default="auto",
                     help="Assembler for each recruited pool (default auto). 'auto' tries direct "
                          "SPAdes first and only adds an Unicycler candidate when SPAdes does not "
                          "already pass the sequence-retention/contiguity gates, then keeps the "
                          "better candidate. Explicit 'unicycler' and 'spades' remain available.")
    ...
    rb.add_argument("--reassemble-mode", choices=["standard", "meta"], default="standard",
                     help="Direct SPAdes mode (default standard, matching the focused Verdanus "
                          "reassembly strategy). Use 'meta' when the recruited pool is expected "
                          "to contain substantial strain/taxon heterogeneity.")
    ...
    simple.add_argument("--rounds", type=int, default=None,
                        help="Guarded frontier-extension rounds (default 1; use 0 for seed-only).")
...
def main(argv=None):
    ...
    # Bowtie2 remains the coverage/link mapper. Competitive seed recruitment can now
    # independently use Bowtie2 or BWA-MEM.
    if refinement_enabled or (args.reassemble_bins and args.expansion_mode == "link"):
    ...
    if args.reassemble_bins:
        ...
        if args.expansion_mode == "reassemble" and (args.assembler in ("auto", "spades") or args.spades_fallback):
            which_or_die("spades.py")
        if args.expansion_mode == "reassemble" and args.assembler in ("auto", "unicycler"):
            unicycler_env_names = [
                e.strip() for e in str(args.unicycler_env).split(",") if e.strip()
            ]
            unicycler_argv = resolve_unicycler_runner(args.unicycler_cmd, unicycler_env_names)
            if unicycler_argv is None and args.assembler == "unicycler":
                log.error(
                    "--assembler unicycler was requested but Unicycler could not be launched. "
                    "Install it, pass --unicycler-cmd, or use --assembler auto/spades."
                )
                sys.exit(1)
            if unicycler_argv is None and args.assembler == "auto":
                log.warning("Unicycler is unavailable; --assembler auto will use direct SPAdes only.")
    ...
    if args.reassemble_bins:
        ...
        successful_reassemblies = run_bin_reassembly(
            ...
            seed_mapper=args.seed_mapper, seed_tie_policy=args.seed_tie_policy,
            bwa_seed_length=args.bwa_seed_length,
            bwa_mismatch_penalty=args.bwa_mismatch_penalty,
            bwa_gap_open_penalty=args.bwa_gap_open_penalty,
