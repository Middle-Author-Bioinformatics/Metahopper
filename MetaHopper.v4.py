...
Entries marked [cond] are only created when that condition holds.
      ...
      bins/                                        <- FINAL bins; assess these
        <rank>/
          ...
          summary.tsv                              <-- per bin: contigs, length, N50, L50,
                                                   ...
                                                   The PRELIMINARY bins/<rank>/ get the
                                                   same table before expansion (unless
                                                   --skip-preliminary-assessment), which
                                                   is what the report's Expansion tab
                                                   compares against.
  ...
  Expansion mode
  --------------
  --expansion-mode defaults to 'link'. Both modes recruit reads the same way -- a
  competitive seed mapping against every bin at the rank, then BBDuk frontier rounds --
  and they differ only in what they do with the pool:

    link         map the pool back to the assembly and ADOPT the existing contigs it
                 covers. Sequence is never altered, so a contig cannot be pruned away
                 and cannot be re-voted into a sibling genus. Only contigs that no
                 named bin already owns are adoptable, and a contig wanted by two bins
                 goes to the better-supported one. Thresholds:
                 --link-min-covered-fraction, --link-min-reads.
    reassemble   run the assembler on the pool and REPLACE the bin's contigs with its
                 output. This can close a circular genome, but Unicycler prunes hard by
                 default and the replacement contigs are new sequence that the final
                 DIAMOND pass re-votes from scratch -- on an unevenly covered symbiont
                 that can shrink the bin and scatter it across sibling genera.

  Bin size floor
  --------------
  --min-bin-length defaults to 50 kb: bins below it are merged into Unclassified. A
  metagenome yields a long tail of genus bins holding a few hundred bp of spurious best
  hits, and because the reassembly stage runs per bin -- extraction, QC, up to
  --reassemble-max-rounds BBDuk rounds, Unicycler and Pilon each time -- that tail
  dominates its runtime. On a real insect metagenome, 180 genus bins fell to 6 at this
  threshold and the stage went from ~18 h to well under one. Pass --min-bin-length 0 to
  keep every bin.
...
def resolve_link_claims(link_claims: dict, owned_ids: set) -> dict:
    """Award each contested contig to exactly one bin.

    ``owned_ids`` are contigs a named bin already holds; those are never adoptable,
    because moving sequence between sibling bins is the drift that linking exists to
    avoid. Everything else goes to the bin with the strongest support, ties broken by
    read count and then by bin name so the result does not depend on dict ordering.
    """
    best = {}
    for bin_name in sorted(link_claims):
        adopted, support, _before, _fasta = link_claims[bin_name]
        for contig_id in adopted:
            if contig_id in owned_ids:
                continue
            score = support[contig_id]
            prev = best.get(contig_id)
            if prev is None or score > prev[1]:
                best[contig_id] = (bin_name, score)
    awarded = defaultdict(set)
    for contig_id, (bin_name, _score) in best.items():
        awarded[bin_name].add(contig_id)
    return awarded
...
def reassemble_one_bin(bin_fasta: Path, bin_name: str, rank: str, r1_raw: Path, r2_raw: Path,
                        ...
                        link_min_reads: int = 20, link_max_insert: int = 1000) -> Path:
...
def run_bin_reassembly(outdir: Path, ranks, r1_raw: Path, r2_raw: Path, threads: int,
                        ...
                        bin_threads: int = None, expansion_mode: str = "link",
                        assembly_fasta: Path = None,
                        link_min_covered_fraction: float = 0.5,
                        link_min_reads: int = 20) -> dict:
    ...
    if expansion_mode == "link":
        if assembly_fasta is None or not Path(assembly_fasta).is_file():
            raise ValueError("--expansion-mode link needs the assembly it binned.")
        link_root = outdir / "reassembly" / "_link_reference"
        link_root.mkdir(parents=True, exist_ok=True)
        assembly_index = link_root / "assembly_index"
        if not Path(str(assembly_index) + ".1.bt2").is_file():
            log.info("Indexing the assembly once so every bin can be linked against it...")
            build_bowtie2_index(Path(assembly_fasta), assembly_index, threads,
                                link_root / "bowtie2-build.log")
        assembly_seqs = read_fasta(Path(assembly_fasta))
    for r in ranks:
        ...
        for bin_fasta in bin_fastas:
            ...
            linked_done = outdir / "reassembly" / r / bin_name / "linked.fasta"
            if (expansion_mode == "link" and resume and linked_done.is_file()
                    and linked_done.stat().st_size > 0):
                # Linked output is original sequence, so reusing it is exact.
                log.info("Bin '%s' (%s): linked.fasta already present; reusing it.",
                         bin_name, r)
                successful[r][bin_name] = linked_done
                after = basic_assembly_stats(linked_done)
                summary_rows.append({
                    "bin": bin_name, "rank": r,
                    "contigs_before": before["num_contigs"],
                    "contigs_after": after["num_contigs"],
                    ...
                    "recovered_fraction": (
                        f"{after['total_length_bp'] / before['total_length_bp']:.4f}"
                        if before["total_length_bp"] else "0.0000"),
                    "outcome": "linked",
                })
                continue
            if resume and finished.is_file() and finished.stat().st_size > 0:
                ...
                try:
                    reassembled = reassemble_one_bin(
                        ...
                        unicycler_argv, expansion_mode=expansion_mode,
                        assembly_index=assembly_index, assembly_seqs=assembly_seqs,
                        link_min_covered_fraction=link_min_covered_fraction,
                    ...
                    reassembled = None
            if expansion_mode == "link":
                # Collected now, resolved once the whole rank has been seen: two bins
                # can want the same contig, and the loser must not silently keep it.
                if reassembled is None:
                    continue
                _cand_path, adopted, support = reassembled
                link_claims[bin_name] = (adopted, support, before, bin_fasta)
                continue
            if reassembled is None:
                continue
        ...
        if expansion_mode == "link" and link_claims:
            # Only sequence that no bin already owns may be adopted. Stealing a contig
            # from a sibling bin is exactly the failure mode linking exists to avoid.
            owned = set()
            for p in all_bin_fastas:
                if p.name != "Unclassified.fasta":
                    owned |= set(read_fasta(p))
            awarded = resolve_link_claims(link_claims, owned)
            for bin_name, (_adopted, support, before, bin_fasta) in link_claims.items():
                gained = awarded.get(bin_name, set())
                out_dir = outdir / "reassembly" / r / bin_name
                out_dir.mkdir(parents=True, exist_ok=True)
                linked = out_dir / "linked.fasta"
                write_linked_bin(bin_fasta, gained, assembly_seqs, linked)
                after = basic_assembly_stats(linked)
                successful[r][bin_name] = linked
                gained_bp = sum(len(assembly_seqs.get(c, "")) for c in gained)
                log.info("Bin '%s' (%s): linked in %d contig(s) / %d bp; %d -> %d "
                         "contigs, %d -> %d bp. No sequence was rebuilt.",
                         bin_name, r, len(gained), gained_bp, before["num_contigs"],
                         after["num_contigs"], before["total_length_bp"],
                         after["total_length_bp"])
                summary_rows.append({
                    "bin": bin_name, "rank": r,
                    "contigs_before": before["num_contigs"],
                    "contigs_after": after["num_contigs"],
...
def parse_args(argv=None):
    ...
    rb.add_argument("--reassemble-threads", type=int, default=None,
                     help="Threads for the per-bin work inside the reassembly stage "
                          ...
                          "scheduler sees.")
    rb.add_argument("--expansion-mode", choices=["link", "reassemble"], default="link",
                     help="What to do with each bin's recruited read pool. 'link' "
                          ...
                          "covered one.")
    rb.add_argument("--link-min-covered-fraction", type=float, default=0.5,
